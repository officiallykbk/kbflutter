...
class CargoJobProvider extends ChangeNotifier {
  ...
  Future<void> fetchJobsData() async {
    try {
      ...
      if (!_isNetworkOffline) {
        // Only set to true and notify if it wasn't already offline
  ...
  void _startReconnectionTimer() {
    ...
    _reconnectionTimer =
      ...
      if (!_isNetworkOffline) {
        // Should not happen if timer is only started when offline
        timer.cancel();
        return;
      }
  ...
  Future<void> addJob(CargoJob job) async {
    ...
    print(
        'Attempting to add job. Payload: $jobData'); // Detailed logging before insert

    try {
      final response = await _supabase
          .from('cargo_jobs')
          .insert(jobData)
          .select(); // Added .select() to get response
      ...
      if (response == null || (response is List && response.isEmpty)) {
        print(
            'Error adding job: Supabase returned no data or empty list, indicating potential issue.');
